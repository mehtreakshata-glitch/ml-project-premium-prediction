...
def  preprocess_input(input_dict):
    ...
    expected_columns = ['age','number_of_dependants', 'income_lakhs','insurance_plan','normalized_risk_score','genetical_risk','gender_Male','region_Northwest','region_Southeast',
                        'region_Southwest','marital_status_Unmarried','bmi_category_Obesity','bmi_category_Overweight','bmi_category_Underweight','smoking_status_No Smoking',
                        'smoking_status_Not Smoking','smoking_status_Occasional','smoking_status_Regular','smoking_status_Smoking=0','employment_status_Salaried','employment_status_Self-Employed'
    ...
    for key, value in input_dict.items():
        if key == 'Gender' and value == 'Male':
            ...
            elif value == 'Not Smoking':
                df['smoking_status_Not Smoking'] = 1
            elif value == 'Smoking=0':
...
def predict(input_dict):
    ...
    if input_dict['Age'] <= 25:
        input_df = input_df.reindex(
            columns=model_young.feature_names_in_,
            fill_value=0
        )
        prediction = model_young.predict(input_df)
    else:
        input_df = input_df.reindex(
            columns=model_rest.feature_names_in_,
            fill_value=0
        )
